...
void ReceptionISR(int sig_int)
{
    rx_buf[0] = (rx_buf[0]<<8)>>8;
    rx_buf[1] = (rx_buf[1]<<8)>>8;

	tx_buf[0]=(int)((float)rx_buf[0]/2);
	tx_buf[1]=(int)((float)rx_buf[1]/2);
}
...
void IRQP1_isr_leds (int sig_num)
{
...
void IRQP1_isr ( int sig_num )
{
    ...
    //tester si c'est bien DAI_P1 qui est � l'origine de l'interruption DAI
    if ((IT_register & SRU_EXTMISCB1_INT) != 0)
    {
...
void main(void)
{
	...
	*pDAI_IRPTL_RE = SRU_EXTMISCB1_INT; // Choisir le front montant pour l'interruption
	...
	interrupt(SIG_DAIH,IRQP1_isr_leds);
	...
	*pSYSCTL |= IRQ1EN;
    ...
    while(1){
        if(start_processing == 1){
            // computations on the processing buffer
